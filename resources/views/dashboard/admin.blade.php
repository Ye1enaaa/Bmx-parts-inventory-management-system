...
<body class="flex h-screen">
    <div id="mySidebar" class="sidebar open">
        <div class="title">
            <a class="flex items-center ms-1">
                <div class="w-12 h-12 rounded-full overflow-hidden">
                    @if(Auth::user()->image)
                    <img src="{{ env('HOST_URL') }}./storage/{{Auth::user()->image}}"
        ...
        <a href="#inventory"
            class="py-2 px-4 text-white hover:bg-blue-400 flex items-center cursor-pointer" onclick="showProducts()">
        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" @click.away="open = false"
                class="dropdown-menu absolute right-0 mt-2 py-2 w-full bg-[#072e33] rounded-md shadow-lg">
                <a class="dropdown-item block px-4 py-2 text-white hover:bg-blue-400 text-xs" href="#"
                    onclick="showSupplierInformation()">
                    Supplier Information
                </a>
                <br>
                <a class="dropdown-item block px-4 py-2 text-white hover:bg-blue-400 text-xs" href="#"
                    onclick="showAddSupplier()">
                    Add Supplier
                </a>
            </div>
        </div>
