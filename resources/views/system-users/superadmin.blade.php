...
<body class="flex h-screen">
  ...
  <div id="main">
    ...
    <div id="content-accounts">
      ...
      <div class="popup-form edit-user-form h-300px w-950px bg-gray-200 px-10 space-y-10 mx-auto p-11 rounded-1xl shadow-md" data-user-id="{{ $user->id }}">
        <form id="edit-user-form-({{ $user->id }})" action="{{ route('edit.user', $user->id) }}" method="post">
          @csrf
          @method('PUT')
          <h1 class="justify-center flex text-3xl font-bold mb-5 text-black"><b>Edit Infomation</b></h1>
          <input type="hidden" name="user_id" value="{{ $user->id }}">
          <label for="name" class="block w-20 mr-2 font-bold dark:text-gray">Name:</label>
          <input class="border-black block py-2 px-4 w-full rounded focus:outline-none focus:border-black" type="text" name="name" value="{{ $user->name }}" required><br>
          <label for="password" class="block w-20 mr-2 font-bold dark:text-gray">Password:</label>
          <input class="border-black block py-2 px-4 w-full rounded focus:outline-none focus:border-black" type="password" name="password" required><br>
          <label for="image" class="block w-20 mr-2 font-bold dark:text-gray">Picture:</label>
          <input type="file" name="image" id="imageInput">
          <div class="flex justify-center">
            <button type="submit" class="mr-2 text-white bg-gray-900 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">SAVE</button>
            <button class="text-white bg-gray-900 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button" onclick="hideEditForm({{ $user->id }})">CANCEL</button>
          </div>
        </form>
      </div>
  ...
  <div id="content-add-accounts">
    <div class="card">
      ...
      <div class="h-900px w-950px bg-gray-200 px-10 space-y-10 mx-auto p-11  shadow-md" >
        <div class="card-body">
